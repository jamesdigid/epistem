...
pub trait ProviderSelector {
    fn select(&self, capability: &str, candidates: Vec<CandidateProvider>) -> Result<CandidateProvider>;
}
...
impl ProviderSelector for DeterministicSelector {
    fn select(&self, capability: &str, candidates: Vec<CandidateProvider>) -> Result<CandidateProvider> {
        for candidate in candidates {
            if !candidate.manifest.capabilities.iter().any(|entry| entry == capability) {
                continue;
            }
